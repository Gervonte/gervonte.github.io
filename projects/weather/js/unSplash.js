class unSplashObject
{
    constructor(main)
    {
        ...
        this.accessKey = "9a8f9223698873cc18279a27b49d61cc7408bda114021668c5b8320fdcded6c9";
        
        //Amount of images to return
        this.perPage = 30;
        
        //Partial Url to be added on to after getting weather
        this.URL = "https://api.unsplash.com/search/photos/?per_page="+this.perPage+"&order_by=popular&query=";       
    }
    ...
    requestData(searchTerm)
    {
        ...
        this.apiRequest.setRequestHeader("Authorization","Client-ID "+this.accessKey);
        ...
        function callback()
        {
            ...
            if(thisInherit.verbose){console.log("Choosing image number: "+thisInherit.mainObject.choice)};
            console.log(thisInherit.data)
            thisInherit.mainObject.image=thisInherit.data.results[thisInherit.mainObject.choice].urls.raw;
        ...
        this.apiRequest.onload = function()
        {
            ...
            if(thisInherit.apiRequest.status >= 200 && thisInherit.apiRequest.status <400)
            {     
                if(thisInherit.verbose){console.log('Unsplash API Request succesful!');}
                callback();
            }
            else
            {
                if(thisInherit.verbose){console.log('Unsplash API Request unsuccesful!');}     
            }            
        };        
