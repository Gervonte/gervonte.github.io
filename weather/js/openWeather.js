class openWeatherObject {
    constructor(main) {
        ...
        this.apiKey = '471ca64f17520039ff239f103b7004d0';

        //URL formed from latitude,longtitude and api key
        this.URL = 'https://api.openweathermap.org/data/2.5/weather?lat=' + this.lat + '&lon=' + this.long + '&APPID=' + this.apiKey;
    }
    ...
    requestData() {
        ...
        this.apiRequest.onload = function () {
            ...
            if (thisInherit.apiRequest.status >= 200 && thisInherit.apiRequest.status < 400) {
                if (thisInherit.verbose) { console.log('OpenWeather API Request succesful!'); }
                callback();
            }
            else {
                if (thisInherit.verbose) { console.log('OpenWeather API Request unsuccesful!'); }
            }
        };
